...
```bash
ticker-master-api$ sam build --use-container
```
...
```bash
ticker-master-api$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
ticker-master-api$ sam local start-api
ticker-master-api$ curl http://localhost:3000/
```
...
```bash
ticker-master-api$ sam logs -n HelloWorldFunction --stack-name ticker-master-api --tail
```
...
```bash
ticker-master-api$ pip install -r tests/requirements.txt --user
# unit test
ticker-master-api$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
ticker-master-api$ AWS_SAM_STACK_NAME="ticker-master-api" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "ticker-master-api"
```
